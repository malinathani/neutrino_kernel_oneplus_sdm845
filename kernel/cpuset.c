					 ...
					 char *buf, size_t nbytes, loff_t off)
{
	...
	struct cpuset *cs = css_cs(of_css(of));
	char *target_cpusets[] =
	{ "audio-app", "top-app", "foreground", "background", "system-background", "restricted" };
	char *target_cpus[] =
	...
	if (!strcmp(current->comm, "init")) {
		for (i = 0; i < ARRAY_SIZE(target_cpusets); i++) {
			if (!strcmp(cs->css.cgroup->kn->name, target_cpusets[i]))
				/* Add '\0' to null-terminate it */
				return cpuset_write_resmask(of, target_cpus[i] += '\0', nbytes, off);
		}
	}
#endif
